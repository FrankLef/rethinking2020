...
plot08_04prior <- within(plot08_04prior, {
  draws <- prior_draws(fit08_04) |>
    mutate(.draw = seq_len(n())) |>
    relocate(.draw)
  
  df <- draws |>
    slice_sample(n = 20) |>
    expand(nesting(.draw, Intercept, b), water_c = -1:1, shade_c = -1:1) |>
    mutate(blooms_r = Intercept + b * water_c,
           label = paste("shade_c =", shade_c))
  
  p <- ggplot(df, aes(x = water_c, y = blooms_r, group = .draw)) +
    geom_line(color = "orchid") +
    geom_hline(
      data = data.frame(y = c(0, 1)), aes(yintercept = y),
      size = 1, linetype = 2, color = "royalblue") +
