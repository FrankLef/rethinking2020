...
Get the data and standardize it. In section 7.1.1 @elreath2020 exolains that we rescale brain size instead of standardizing it because *we wnat to preserve zero* as a reference point.
...
```{r}
fn <- list.files(path='.\\cache', pattern="ch06_b06_06_.*[.]rds$")
stopifnot(length(fn) == 1)
b6.6 <- readRDS(here::here("cache", fn))
fn <- list.files(path='.\\cache', pattern="ch06_b06_07_.*[.]rds$")
stopifnot(length(fn) == 1)
b6.7 <- readRDS(here::here("cache", fn))
fn <- list.files(path='.\\cache', pattern="ch06_b06_08_.*[.]rds$")
stopifnot(length(fn) == 1)
...
```{r}
fn <- list.files(path='.\\cache', pattern="ch05_b05_01_.*[.]rds$")
stopifnot(length(fn) == 1)
b5.1 <- readRDS(here::here("cache", fn))
fn <- list.files(path='.\\cache', pattern="ch05_b05_02_.*[.]rds$")
stopifnot(length(fn) == 1)
b5.2 <- readRDS(here::here("cache", fn))
fn <- list.files(path='.\\cache', pattern="ch05_b05_03_.*[.]rds$")
stopifnot(length(fn) == 1)
