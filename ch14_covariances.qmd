...
> With real data, you never know what the right anser is. This is why studying simulated examples is so important.
...
::: callout-tip
Kurtz says that there is no known way to use `brms`. The R package `bisonR`
specializes in social networks and uses `brms`. It can be found at [bison](https://jhart96.github.io/bisonR/index.html). This section is done with the `bisonR` package.
:::

::: callout-important
I have tried, ***and failed***, to use `bisonR` to do this section.  This section is
incomplete and probably erroneous.  You might want to try and do it 
if the subject of social network interests you. The best source
I found on how to use `bisonR` with a variety of examples is at [bison examples](https://github.com/JHart96/bison_examples/blob/main/examples/dyadic_regression_stan.md)
:::
...
::: callout-warning
Make sure that you have `cmdstanr` version 0.8 or greater. Otherwise, `bisonR::bison_model()` returns an error caused by `cmdstanr`.
:::

::: callout-note
The following is a test to ensure that `bisonR` works properly.
:::

```{r}
tmp <- list()
tmp$sim_data <- bisonR::simulate_bison_model("binary", aggregated = FALSE)
tmp$df <- tmp$sim_data$df_sim
glimpse(head(tmp$df))
```

The path to `cmdstan` has been set in the `CMDSTAN` environment variable. See details of `set_cmdstan_path()` for more info

```{r}
cmdstanr::cmdstan_path()
# cmdstanr::set_cmdstan_path(path = r"(C:\Users\Ephel\Documents\.cmdstan\cmdstan-2.35.0)")
```

```{r}
#| label: "ch14_bison_prior"
tmp$priors <- bisonR::get_default_priors("binary")
```

```{r}
#| eval: false
#| label: "ch14_bison_fit"
tictoc::tic(msg = sprintf("run time of %s, use the cache.", "2 min."))
tmp$fit_edge <- xfun::cache_rds({
  bisonR::bison_model(
  formula = (event | duration) ~ dyad(node_1_id, node_2_id),
  data = tmp$df,
  model_type = "binary",
  priors = tmp$priors,
  mc_cores = getOption("mc_cores", default = 2))},
  file = "ch14_bison_fit", rerun = FALSE)
tictoc::toc()


# tmp$fit_edge <- bisonR::bison_model(
#   formula = (event | duration) ~ dyad(node_1_id, node_2_id),
#   data = tmp$df,
#   model_type = "binary",
#   priors = tmp$priors,
...
```{r}
summary(tmp$fit_edge)
```
::: callout-note
Now that we have ensured `bisonR` works properly, lets go back to *Rethinking*.
:::
...
The research can be find in this [paper](https://research-information.bris.ac.uk/ws/portalfiles/portal/41942591/koster2014sn.pdf). However I could not find what the variable $dlndist$ is. I suppose it it the log of some distance ... but why is it negative? It doesn't matter, it is only used to color the edges of the network and is not used in the analysis.
...
dataKL$dyads <- dataKL$dyads |>
  ...
  # the node must be factors with bisonR package
...
and since this a social network then we can visualize the network with the `ggnetwork` package. For details see [ggnetwork](https://cran.r-project.org/web/packages/ggnetwork/vignettes/ggnetwork.html).
...
```{r}
message("The duration variable is required to use bisonR" )
dataKL$dyads <- dataKL$dyads |>
  mutate(duration = 1)
# glimpse(dataKL$dyads)
```

```{r}
fit14_07_priors <- bisonR::get_default_priors(model_type = "count")
fit14_07_priors
```
and the prior can be visualized

```{r}
prior_check(fit14_07_priors, "count")
```
...
```{r}
#| eval: false
#| label: "ch14_fit14_07_edge"
fit14_07_edge <- bisonR::bison_model(
  data = dataKL$dyads,
  formula = (giftsAB| duration) ~ dyad(hidA, hidB),
  directed = FALSE,
  model_type = "count",
  priors = fit14_07_priors,
...
```{r}
#| eval: false
summary(fit14_07_edge)
```

```{r}
#| eval: false
plot_predictions(fit14_07, num_draws=20, type="density")
```

```{r}
#| eval: false
fitdyadic14_07 <- bisonR::bison_brm(
  data_list = dataKL$dyads,
  formula = giftsAB ~ bison(edge_weight(hidA, hidB)),
  edgemodel_list = fit14_07,
  chains = 2,
...
where the term $\delta_{ij}\sigma^2$ is used when $i=j$, see p. 470 of @elreath2020. In the oceanic case it is not applicable since we have only one observation per society. Therefore the equation is this case is
...
and to plot is we use the `ggtree` package. Details on `ggtree` can be found at [ggtree](https://yulab-smu.top/treedata-book/index.html). It is not available on CRAN at the time of writing this. You can find it in github and install it with `devtools::install_github("GuangchuangYu/ggtree")`.
