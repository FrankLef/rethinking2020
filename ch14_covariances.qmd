...
and to plot is we use the `ggtree` package. Details on `ggtree` can be found at
[ggtree](https://yulab-smu.top/treedata-book/index.html). It is not available
...
We begin by fitting a naive model that does not take into account the phylogenetic
covariances.
...
fit14_09
```
We use the `ape` package to compute the implied covariance matrix and 
distance matrix
