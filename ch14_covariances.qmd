...
The plot is slightly different than what McElreath has. The open circle represent
the actual results and the solid circles are the predicted mean.
...
> With real data, you never know what the right anser is. This is why studying 
...
::: callout-important
I couldn't find the way to replicate this section with `bisonR`. This section
is to be completed later.
:::
...
[bison](https://jhart96.github.io/bisonR/index.html). This section is done with 
the `bisonR` package.
:::


::: callout-warning
Make sure that you have `cmdstanr` version 0.8 or greater. Otherwise,
`bisonR::bison_model()` returns an error caused by `cmdstanr`.
:::
...
The path to `cmdstan` has been set in the `CMDSTAN` environment variable. See
details of `set_cmdstan_path()` for more info
...
```
```{r}
message("The duration is required to use bisonR" )
dataKL$dyads <- dataKL$dyads |>
  mutate(duration = 1)
# glimpse(dataKL$dyads)
```
