...
```{r}
tmp <- length(unique(simCafes$data$cafe))
simCafes$data |>
...
simInstrument <- within(simInstrument, {
  ...
  # df <- simpr::specify(
  #   U = ~ rnorm(n = 500),
  #   Q = ~ scale(sample(x = 1:4, size = 500, replace = TRUE)),
  #   E = ~ scale(rnorm(n = 500, mean = U + Q, sd = 1)),
  #   W = ~ scale(rnorm(n = 500, mean = U + 0 * E, sd = 1))) |>
  #   simpr::generate(1) |>
  #   unnest(sim)
...
Kurtz says that there is no known way to use `brms`. The package `bisonR` that specializes in social networks and uses `brms`. It can be found at [bison](https://github.com/JHart96/bisonR) with a useful vignette at [vignette](https://jordanhart.co.uk/bisonR/articles/getting_started.html).
