...
widths <- c(0.89, 1)
probs <- (1 - widths) / 2
probs <- sort(c(probs, 0.5, 1 - probs))
probs
```
...
plot04_01 <- within(plot04_01, {
  widths <- c(0.89, 1)
  probs <- (1 - widths) / 2
  probs <- sort(c(probs, 0.5, 1 - probs))
  post <- tidy_draws(fit04_01brm)
})

plot04_01 <- within(plot04_01, {
  breaks <- quantile(post$b_Intercept, probs = probs, names = FALSE)
  intercept <- post |>
    ggplot(aes(x = b_Intercept)) +
    stat_halfeye(aes(fill=after_stat(cut_cdf_qi(cdf, .width = widths))),
                 point_interval = mean_qi, .width = widths) +
...
plot04_01 <- within(plot04_01, {
  breaks <- quantile(post$sigma, probs = probs, names = FALSE)
  sigma <- post |> 
    ggplot(aes(x = sigma)) +
    stat_halfeye(aes(fill = after_stat(cut_cdf_qi(cdf, .width = widths))),
                 point_interval = mean_qi, .width = widths) +
...
plot04_01$intercept + plot04_01$sigma +
...
```{r}
post04_03brm$wide <- fit04_03brm |>
  tidybayes::spread_draws(b_Intercept, b_weight_c, sigma)
# glimpse(post04_03brm$wide)
```

```{r}
plot04_03brm$pairs <- GGally::ggscatmat(post04_03brm$wide, 
...
```{r}
pred04_05brm <- list()
pred04_05brm <- within(pred04_05brm, {
  newdata <- data.frame(
    weight_s=seq_range(dataHowel$weight_s, n = 30L)) |>
    mutate(weight_s2 = weight_s^2)
  intrvl <- predicted_draws(fit04_05brm, newdata = newdata, ndraws = 500) |>
    median_qi()
})

lpred04_05brm <- list()
lpred04_05brm <- within(lpred04_05brm, {
  newdata <- data.frame(
    weight_s=seq_range(dataHowel$weight_s, n = 30L)) |>
    mutate(weight_s2 = weight_s^2)
  intrvl <- linpred_draws(fit04_05brm, newdata = newdata, ndraws = 500) |>
    median_qi()
})
```
...
```{r}
lpred04_08brm <- list()
lpred04_08brm <- within(lpred04_08brm, {
  newdata <- dataCherry_nonaB
  draws <- linpred_draws(fit04_08brm, newdata = newdata)
  intrvl <- draws |>
    ungroup() |>
    select(-B) |>
    group_by(year) |>
...
plot04_08brm <- within(plot04_08brm, {
  ...
  lpred <- ggplot(lpred04_08brm$intrvl, aes(x = year, y = .linpred)) +
