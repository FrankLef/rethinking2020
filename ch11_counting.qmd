...
```{r}
plot11_04all <- within(plot11_04all, {
  dataPredicted <- expand_grid(actor = unique(dataChimp$actor),
                               treatment = unique(dataChimp$treatment)) |>
    # dataPredicted <- dataChimp |>
    add_predicted_rvars(fit11_04) |>
    mutate(actor_id = paste("actor", actor),
  ...
  plotPredicted <- ggplot(dataPredicted,
    ...
    geom_point() +
    geom_hline(yintercept = 0.5, color = "brown", linetype = 2) +
    ggrepel::geom_text_repel(aes(label = round(.prediction, 2)), size = 3) +
    scale_y_continuous(labels = scales::label_percent()) +
    coord_cartesian(ylim = c(0, 1)) +
    theme(legend.position = "bottom") +
...
# glimpse(plot11_04all$dataPredicted)
plot11_04all$plotPredicted
```


 and if we compare th emodels
