...
post11_04 <- within(post11_04, {
  ...
  diff <- fit11_04 |> 
    ...
    transmute(ARvsPR = AR - PR,
           ALvsPL = AL - PL) |>
    pivot_longer(cols = c("ARvsPR", "ALvsPL"), 
                 names_to = "treatment",
                 values_to = "estimate") |>
    group_by(treatment) |>
    mean_qi(estimate) |>
...
plot11_04 <- within(plot11_04, {
  ...
  diff_logit <- ggplot(post11_04$diff,
                  aes(x = estimate, xmin = .lower, xmax = .upper, y = treatment)) +
    geom_pointinterval(color = "mediumpurple", linewidth = 2, fatten_point = 3) +
    geom_vline(xintercept = 0, color = "brown") +
    ggrepel::geom_text_repel(mapping = aes(label = round(estimate, 2))) +
    theme(panel.grid.major.y = element_line(color = "white")) +
